...
InModuleScope PoshArmDeployment {
    Describe "Add-ArmApplicationInsightsMetricAlertCriteria" {
        ...
        Context "Unit tests" {
            It "Given valid mandatory params it returns '<Expected>'" -TestCases @(
                ...
                $actual = $MetricAlert
                for ($i = 0; $i -lt $NumberOfCriteria; $i++) {
                    $actual = $actual | Add-ArmApplicationInsightsMetricAlertCriteria -Name $Name `
                        -MetricName $MetricName `
                        -MetricNamespace $MetricNamespace `
                        -Dimensions $Dimensions `
                        -Operator $Operator `
        ...
        Context "Integration tests" {
            It "Default" -Test {
                Invoke-IntegrationTest -ArmResourcesScriptBlock `
                {
                    ...
                    | Add-ArmApplicationInsightsMetricAlertCriteria -Name $ExpectedName `
                        -MetricName $ExpectedMetricName `
                        -MetricNamespace $ExpectedMetricNamespace `
                        -Dimensions $ExpectedDimensions `
                        -Operator $ExpectedOperator `
